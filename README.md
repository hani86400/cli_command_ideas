...
#### SHELL_DATE_IDEA_0001 ( date command samples )
...
#### SHELL_COLORS_IDEA_0002 ( Show colors on terminal )
```bash
show_colors(){
...
#### SHELL_ARGUMENTS_IDEA_0002 ( Use shift to consume positional parameters )
```bash
NNAME=$1; shift
R1=$1; shift
R2=$1; shift
...
#### SHELL_ARGUMENTS_IDEA_0003 ( ${!1} is Value of parameter name $1 )
```bash
echo "Arg1_Name=$1 and Arg1_Value=${!1}" # Arg1_Name=HOME and Arg1_Value=/home/user1
```

#### SHELL_FUNCTION_IDEA_0004 ( Single line function must end with ; } )
```bash
function hello1() {  echo "Hello, World!"; } # (end with ; })
```


#### SHELL_CODE_IDEA_0005 ( strings comparing )
...
#### SHELL_CODE_IDEA_0006 ( Run if variable contains a specific string )
```bash
...
#### SHELL_CODE_IDEA_0007 ( Run debend on the previous command status. )
```bash
echo "" # command
...
#### SHELL_COMMENTS_IDEA_0008 ( Multiline shell script comments )
```bash
# using (: ')
: '
...
#### SHELL_SUDO_IDEA_0009 ( Change system file content which need root privilege. )
```bash
# Overwite
...
#### SHELL_PYTHON_IDEA_0010 ( call Python script or Python function with arguments. )
```bash
